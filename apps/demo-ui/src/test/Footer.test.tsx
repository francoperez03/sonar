...
describe('Footer', () => {
  ...
  it('does not render a dead Run again CTA', async () => {
    const { Footer } = await loadShell();
    render(<Footer />);
    expect(screen.queryByTestId('run-again')).toBeNull();
  });

  it('renders rotation-in-flight status while a rotation is active', async () => {
    ...
    expect(screen.getByText('Rotation in flight')).toBeInTheDocument();
    expect(screen.queryByTestId('run-again')).toBeNull();
  });
