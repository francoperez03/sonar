...
test.describe("Accessibility", () => {
  ...
  test("primary CTA is keyboard-focusable with visible focus ring", async ({ page }, testInfo) => {
    ...
    await page.goto("/");
    // Scope to hero: the locked copy "Watch the 90s demo" appears twice
    // (hero CTA + demo section CTA). Hero owns the primary tabbable target.
    const cta = page.getByTestId("hero").getByText("Watch the 90s demo");
    ...
    // expect cyan outline (#22D3EE → rgb(34, 211, 238))
    expect(outline).toMatch(/rgb\(34,\s*211,\s*238\)/);
  });
});
